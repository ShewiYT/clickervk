...
function UpgradeTab(props) {
	...
	if (active === 'click') {
		return (   
            <Group title="Авто-майнинг">
			<Div>
			<FormStatus title="Обратите внимание" state="error">
			Улучшения увеличивают скорость автоматического майнинга.
			</FormStatus>
			</Div>
	            <SetCellClicks upgrades={props.upgradesTwo} buyUpgrade={props.buyUpgrade} />
	        </Group>
        );
...
const Upgrade = props => (
	<Panel id={props.id}>
		...
		<UpgradeTab activeTab={props.activeTab} buyUpgrade={props.buyUpgrade} upgradesTwo={props.upgradesTwo} upgradesTwo={props.upgradesTwo} />
	</Panel>
);
